...
export function createMatch3Sound({ surface, durationMS }) {
  ...
  let context = null, master = null, buffers = null, resumePending = null, destroyed = false, visible = !document.hidden, lastTransition = null, pendingSnapshot = null;
  ...
  function unlock() {
    ...
    if (context === null) {
      ...
      queueMicrotask(() => {
        ...
        try { buffers = createBuffers(context, durationMS / 1_000); playPending(); }
        catch { buffers = null; pendingSnapshot = null; }
      });
    }
  ...
  function sync(snapshot) {
    ...
    if (snapshot?.operation !== OPERATION_RESOLVE || !transition) { pendingSnapshot = null; return; }
  ...
  function playPending() {
    if (destroyed || !visible || !pendingSnapshot || !context || context.state !== "running" || !buffers) return;
    const snapshot = pendingSnapshot, transition = snapshot.transition;
    const key = `${transition.startGT}:${transition.endGT}`;
    if (key === lastTransition) { pendingSnapshot = null; return; }
    try {
      ...
      source.start(0, offset); active.add(source); lastTransition = key; pendingSnapshot = null;
    } catch {}
  }
  ...
  return Object.freeze({
    ...
    setVisible(value) {
      ...
      if (!visible) { pendingSnapshot = null; stop(); context?.suspend().catch(() => {}); }
      else if (context && context.state !== "running" && context.state !== "closed") requestResume();
      else playPending();
    },
    destroy() {
      if (destroyed) return; destroyed = true; pendingSnapshot = null; stop(); removeUnlockListeners();
      if (context && context.state !== "closed") context.close().catch(() => {});
      context = null; master = null; buffers = null; resumePending = null;
    },
  });
}
